...
object StringSolver {
  ...
  trait ProblemSimplicationPhase {
    self =>
          ...
          case (sentence@(ids, rhs)) :: q => // If the constants have an unique position, we should split the equation.
            ...
            if (constants.nonEmpty) {
              ...
              if (invalid) None
                ...
                if (i == -1) {
                  // OK it's the smallest position possible, hence we can split at the last position.
  ...
  object PropagateEquations extends ProblemSimplicationPhase {
    def run(p: Problem, s: Assignment): Option[(Problem, Assignment)] = {
      var newP = p.distinct
      for ((lhs, rhs) <- newP if lhs.length >= 2) {
        // Original distinct p.
        var indexInP = 0
    ...
    ids match {
      ...
      case x :: ys =>
        ...
        if (worstScore == bestScore) {
          ...
          // A variable appears more than others in the same equation, so its changes are going to propagate more.
          val indexBestVar = ids.indexOf(bestVar)
          val strings = if (indexBestVar == 0) {
            // Test only string prefixes or empty string
...
//    println(Thread.currentThread.getStackTrace.length.toString + " Problem:\n"+renderProblem(p))
...
//    }
  ...
  def solveGeneralProblem(b: GeneralProblem): Stream[Assignment] = {
    ...
    if (unbounded == Nil) solve(bounded)
    else
...
//    */
//  def keepEachOccurenceSeparatlyAndThenAllOfThem(p: Problem, ifVariable:
//  Set[Identifier], initialMapping: Assignment): Stream[Problem] = {
...
//    * It will try deletions from the end and from the start of one variable.
//    * */
//  def minimizeChanges(s: Stream[Assignment], p: Problem, ifVariable:
//  Set[Identifier], initialMapping: Assignment): Stream[Assignment] = {
//    if (s.isEmpty || ifVariable.size <= 1) s
//    else {
//      ((for {v <- ifVariable.toStream
//             originalValue = initialMapping(v)
//             i <- originalValue.length to 0 by -1
//             prefix <- (if (i == 0 || i == originalValue.length) List(true)
//             else List(true, false))
//             possibleValue = (if (prefix) originalValue.substring(0, i)
//             else
//               originalValue.substring(originalValue.length - i))
//             s <- solve(reduceProblem(Map(v -> possibleValue))(p))
//      } yield s + (v -> possibleValue)) ++ s).distinct
//    }
//  }
//
//  /** Solves the problem while supposing that a minimal number of variables
//    * have been changed.
//    * Will try to replace variables from the left first.
//    * If a variable occurs multiple times, will try to replace each of its
//    * occurrence first.
//    * */
//  def solveMinChange(p: Problem, initialMapping: Assignment):
//  Stream[Assignment] = {
//    // First try to see if the problem is solved. If yes, returns the initial mapping
//    val initKeys = initialMapping.keys.toSeq
//    for {
//      i <- (0 to initialMapping.size).toStream
//      replaceAllButOneOccurence <- /*Seq*/Stream(false, true) // If false, does not modify the equation. If true, replaces every but one occurence of each variable with its original value.
//      toReplace <- take(i, initKeys)
//      ifVariable = toReplace.toSet
////      _ = println("replaceAllButOneOccurence = "+replaceAllButOneOccurence+"\ntoReplace = "+ toReplace)
//      newProblemReduced = reduceProblem(initialMapping filterKeys (x => !ifVariable(x)))(p)
//      newProblemsMixed = if (replaceAllButOneOccurence)
//        keepEachOccurenceSeparatlyAndThenAllOfThem(newProblemReduced,
//          ifVariable, initialMapping)
//      else Stream(newProblemReduced)
//      newProblem <- newProblemsMixed
//      solution <- minimizeChanges(solve(newProblem), newProblem,
//        ifVariable, initialMapping: Assignment)
//    } yield solution
//  }
//}
//
